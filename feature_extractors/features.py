...
class Features(torch.nn.Module):
    ...
    def run_late_fusion(self):
        ...
        for scores in self.s_lib:
            scores = scores.float()
            fused = self.gate(scores)
            gated_scores.append(fused.unsqueeze(0))

        gated_scores = torch.cat(gated_scores, 0).detach().cpu().numpy()

        self.detect_fuser.fit(gated_scores)
